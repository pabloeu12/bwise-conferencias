...
import { FileSpreadsheet, Banknote, CalendarDays, WalletCards } from "lucide-react";
        ...
        <div className="p-6 md:p-12 flex-1 z-10 overflow-y-auto">
          <div className="max-w-7xl mx-auto relative">
            ...
            <div>
              ...
              {/* GRID COM OS 4 CARDS */}
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8 xl:gap-10 text-left">
                ...
                {/* CARD 4: CONSIGNADOS */}
                <div onClick={() => router.push("/consignados")} className="bg-[#1e212b] rounded-2xl p-6 shadow-xl hover:shadow-2xl hover:-translate-y-3 transition-all duration-300 flex flex-col justify-between cursor-pointer group min-h-[340px]">
                  <div>
                    <div className="flex items-center gap-4 mb-4 pb-3 border-b border-slate-700/50">
                      <div className="relative p-3 bg-emerald-500/10 rounded-xl text-emerald-400 flex items-center justify-center border border-emerald-500/20 shrink-0">
                        <div className="absolute inset-0 bg-gradient-to-br from-emerald-400 to-transparent opacity-30 blur-md rounded-full pointer-events-none"></div>
                        <WalletCards className="w-6 h-6 relative z-10" />
                      </div>
                      <h4 className="text-white font-bold text-lg xl:text-xl leading-snug">Empréstimos Consignados</h4>
                    </div>
                    <p className="text-slate-300 text-sm leading-relaxed">Cruzamento de relatórios Emprega Brasil contra sistema de Folha. Validação rígida da margem limite de 35% e identificação automática de divergências de lançamentos em lote.</p>
                  </div>
                  <div className="text-emerald-400 text-xs font-bold mt-6 pt-3 border-t border-slate-700/50 inline-flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                    Acessar Módulo ➔
                  </div>
            ...
            <div className="mt-32 pt-16 border-t border-slate-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                ...
                <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm flex flex-col justify-between space-y-4">
                  <div>
                    <h4 className="text-slate-800 font-bold text-base mb-2 flex items-center gap-2"><span className="text-xl">🛠️</span> Demandas sob Medida & Automações</h4>
                    <p className="text-slate-500 text-xs leading-relaxed">Sua empresa opera com regras específicas ou necessita de integração com outras plataformas? Desenvolvemos rotinas exclusivas para o seu fluxo de Departamento Pessoal.</p>
                  </div>
                  ...
                  <a href="mailto:comercial@bwisecontabilidade.com?subject=Solicitação de Orçamento - Novos Módulos de DP" className="w-full text-center py-2.5 bg-[#1e212b] hover:bg-slate-800 text-white font-bold text-xs rounded-xl transition-colors tracking-wide uppercase shadow-sm">Solicitar Orçamento de Novos Módulos</a>
                </div>
                {/* SUPORTE */}
                <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm flex flex-col justify-between space-y-4">
                  <div>
                    <h4 className="text-slate-800 font-bold text-base mb-2 flex items-center gap-2"><span className="text-xl">📞</span> Suporte Operacional da Plataforma</h4>
                    <p className="text-slate-500 text-xs leading-relaxed">Instabilidades no motor de processamento, dúvidas de parametrização ou erros de upload em lote devem ser reportados diretamente ao time técnico.</p>
                  </div>
